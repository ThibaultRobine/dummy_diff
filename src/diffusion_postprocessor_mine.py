...
class CustomDiffusionPostprocessor(BasePostprocessor):
    ...
    def setup(self, net, id_loader_dict, ood_loader_dict):
        ...
        self.x_ref = optimize_reference_point(
            ...
            device='cuda',
            convergence_window=self.integration_args.get('asc_convergence_window'),
            convergence_threshold=self.integration_args.get('asc_convergence_threshold'),
            min_steps=self.integration_args.get('asc_min_steps'),
        )
        ...
        self.kde, self.scaler = train_kde(torch.cat(scores_list, dim=1))
    ...
    def postprocess(self, net: torch.nn.Module, data: Any):
        ...
        scaled_scores = self.scaler.transform(
            torch.cat(all_scores_list, dim=1).cpu().numpy()
        )
