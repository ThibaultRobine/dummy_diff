# src/msma_postprocessor.py
from typing import Any
import torch
import numpy as np
from openood.postprocessors import BasePostprocessor
from algorithm import compute_msma_stats, train_kde
...
class MSMAPostprocessor(BasePostprocessor):
    def __init__(self, config):
        ...
        self.msma_args = post_cfg.get('msma_args', {})
    ...
    def setup(self, net, id_loader_dict, ood_loader_dict):
        ...
        for batch_data in id_loader_dict['train']:
            with torch.no_grad():
                ...
                stats = compute_msma_stats(
            model=self.diffusion_model,
            diffusion=self.diffusion_obj,
            data=feats_b,
            n_steps=self.msma_args.get('n_steps'),
            batch_size=self.msma_args.get('batch_size'),
            device=self.msma_args.get('device', 'cuda')
        )
    ...
    def postprocess(self, net: torch.nn.Module, data: Any):
        ...
        return pred, torch.from_numpy(kde_scores).to(data.device)
