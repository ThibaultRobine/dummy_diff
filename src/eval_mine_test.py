...
from diffusion_postprocessor_mine_test import CustomDiffusionPostprocessorTest
...
def main():
    ...
    config = {
        ...
        'time_embedding_dim': 1024
            ...
            'integration_method': {
                ...
                'asc_convergence_window': 500,
                'asc_convergence_threshold': 1e-1,
                'asc_min_steps': 100,
    ...
    checkpoint = torch.load('model_checkpoint_latent_linear.pth')
    ...
    custom_postprocessor = CustomDiffusionPostprocessorTest(config)
    print("Running evaluation...")
    cusotom_evaluator = Evaluator(
        net,
        id_name=ID_NAME,
        data_root=DATA_ROOT,
    ...
    metrics = cusotom_evaluator.eval_ood(fsood=False)
    # Save results
    print('\nFinal metrics:')
    print(metrics)
    pd.DataFrame(metrics).to_csv('ood_results_test_ours.csv', index=False)
    print("Results saved to ood_results_test_ours.csv")
    ...
    # print("Results saved to ood_results_ddpm_impr.csv")
    ...
    # print('\nFinal metrics:')
    # print(metrics)
    # pd.DataFrame(metrics).to_csv('ood_results_test_impr.csv', index=False)
    # print("Results saved to ood_results_test_impr.csv")
    ...
    # full_results.to_csv('ood_results_full.csv')
    # print("All results saved to ood_results_full.csv")
    # print(full_results)
